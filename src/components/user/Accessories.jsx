...
const Accessories = () => {
  ...
  // ✅ Accessories category + Active Filter logic
  const filteredProducts = products.filter((product) => {
    const isAccessories = product.categoryId?.name === "Accessories";
    if (activeFilter === "All") return isAccessories;
    return isAccessories && product.name.toLowerCase().includes(activeFilter.toLowerCase());
  });

  return (
    <div className="bg-[#FFF0F5] min-h-screen p-6">
      <h1 className="text-2xl font-bold mb-6">Accessories Collection</h1>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-52 bg-white p-4 rounded-xl shadow-sm h-fit border border-pink-100">
          ...
          <div className="flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0">
            {["All", "Watch", "Belt", "Wallet", "SunGlass", "Cap"].map((item) => (
              <button
                key={item}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full text-center py-20 bg-white rounded-xl border border-dashed border-gray-300 text-gray-500">
                Accessories ma "{activeFilter}" category ma haji products nathi.
              </div>
            )}
          </div>
        </div>
